...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `YouTube agent returned ${response.status}${body ? `: ${body.slice(0, 200)}` : ""}`,
    );
  }
...
export async function generateVideo(input: {
  ...
  style?: string;
  length?: "short" | "medium" | "long";
}): Promise<YouTubeControlResult> {
  const info = await youtubeAgentInfo();
  if (!info.reachable) {
    return {
      ok: false,
      reply: "The YouTube agent is not running. Start it, then ask me again.",
      error: info.error,
    };
  }
  if (info.health?.setupRequired) {
    return {
      ok: false,
      reply:
        "The YouTube agent is in setup mode — it needs YouTube credentials before it can generate. Run `npm run walkthrough` in the agent directory.",
  ...
  try {
    const { ok, result } = await request<{
      ok: boolean;
      result: { job: { id: string; topic: string; status: string } };
    }>("/generate", {
      method: "POST",
      body: JSON.stringify({
    ...
    return {
      ok: false,
      reply: `Could not start generation: ${error instanceof Error ? error.message : "unknown error"}.`,
      error: String(error),
    };
  }
}
...
async function getJob(jobId: string): Promise<YouTubeControlResult> {
  try {
    ...
    if (!job?.id)
      return {
        ok: false,
        reply: "That job was not found on the YouTube agent.",
        error: "not_found",
      };
    const progress =
    ...
    return {
      ok: false,
      reply: `Could not read job: ${error instanceof Error ? error.message : "unknown error"}.`,
      error: String(error),
    };
  }
}
...
async function listJobs(): Promise<YouTubeControlResult> {
  try {
    const data = await request<{
      jobs?: Array<{ id: string; status: string; topic: string }>;
    }>("/api/dashboard");
    const jobs = data.jobs ?? [];
    if (!jobs.length)
      return {
        ok: true,
        reply: "There are no generation jobs on the YouTube agent yet.",
      };
    const summary = jobs
      .slice(0, 10)
      .map(
        (job) => `• ${job.id} — ${job.status} — ${job.topic || "auto topic"}`,
      )
      .join("\n");
    ...
    return {
      ok: false,
      reply: `Could not list jobs: ${error instanceof Error ? error.message : "unknown error"}.`,
      error: String(error),
    };
  }
}
...
async function publishContent(
  ...
  try {
    ...
    return {
      ok: false,
      reply: `Could not publish: ${error instanceof Error ? error.message : "unknown error"}.`,
      error: String(error),
    };
  }
}
...
async function inspectPublishable(
  ...
  try {
    ...
    return {
      ok: false,
      reply: `Could not read content: ${error instanceof Error ? error.message : "unknown error"}.`,
      error: String(error),
    };
  }
}
...
async function approveContent(
  ...
  try {
    const data = await request<{
      ...
      result?: unknown;
      error?: string;
    }>(`/api/content/${encodeURIComponent(contentId)}/approve`, {
      method: "POST",
      body: JSON.stringify({}),
    });
    ...
    return {
      ok: true,
      reply: `Content ${contentId} was approved. It is now eligible for publishing.`,
      data: data.result,
    };
  } catch (error) {
    return {
      ok: false,
      reply: `Could not approve: ${error instanceof Error ? error.message : "unknown error"}.`,
      error: String(error),
    };
  }
}
...
async function rejectContent(contentId: string): Promise<YouTubeControlResult> {
  try {
    const data = await request<{
      ...
      result?: unknown;
      error?: string;
    }>(`/api/content/${encodeURIComponent(contentId)}/reject`, {
      method: "POST",
      body: JSON.stringify({}),
    });
    ...
    return {
      ok: true,
      reply: `Content ${contentId} was rejected and will not be published.`,
      data: data.result,
    };
  } catch (error) {
    return {
      ok: false,
      reply: `Could not reject: ${error instanceof Error ? error.message : "unknown error"}.`,
      error: String(error),
    };
  }
}
...
async function operatorStart(): Promise<YouTubeControlResult> {
  try {
    const data = await request<{
      success: boolean;
      result?: { run?: { id: string; status: string } };
      error?: string;
    }>("/api/operator/start", {
      method: "POST",
      body: JSON.stringify({}),
    ...
    return {
      ok: true,
      reply: `Autonomous operator started${run && "id" in run ? ` (run ${run.id})` : ""}. It will research, plan, and produce videos on its own.`,
      data: data.result,
    };
  } catch (error) {
    return {
...
async function operatorPause(): Promise<YouTubeControlResult> {
  try {
    ...
    return {
      ok: true,
      reply:
        "Autonomous operator paused. Scheduled automation is stopped until you resume it.",
      data: data.result,
    };
  } catch (error) {
    return {
      ok: false,
      reply: `Could not pause: ${error instanceof Error ? error.message : "unknown error"}.`,
      error: String(error),
    };
  }
}
  ...
  try {
    ...
    if (!ideas.length)
      return {
        ok: true,
        reply:
          "There are no content ideas on the agent yet — ask me to research some, or start the operator.",
      };
    const summary = ideas
      .slice(0, 8)
      .map(
        (idea) =>
          `• ${idea.id} — ${idea.status} — ${idea.topic}${idea.angle ? ` (${idea.angle})` : ""}`,
      )
      .join("\n");
    ...
    return {
      ok: false,
      reply: `Could not list ideas: ${error instanceof Error ? error.message : "unknown error"}.`,
      error: String(error),
    };
  }
}
...
async function analyticsReport(): Promise<YouTubeControlResult> {
  try {
    ...
    return {
      ok: false,
      reply: `Could not fetch analytics: ${error instanceof Error ? error.message : "unknown error"}.`,
      error: String(error),
    };
  }
}
...
async function strategyReport(): Promise<YouTubeControlResult> {
  try {
    ...
    const strategy =
      data.strategy ??
      (data as unknown as { channelStrategy?: Record<string, unknown> })
        .channelStrategy;
    if (
      !strategy ||
      ...
      return {
        ok: false,
        reply:
          "No channel strategy is saved yet. Ask me to research and plan one, or set it in the agent dashboard.",
        error: "no_strategy",
      };
    }
    const { objective, audience, content_pillars, cadence_per_week } =
    ...
    return {
      ok: false,
      reply: `Could not fetch strategy: ${error instanceof Error ? error.message : "unknown error"}.`,
      error: String(error),
    };
  }
}
...
async function scheduleReport(): Promise<YouTubeControlResult> {
  try {
    ...
    if (!items.length)
      return { ok: true, reply: "Nothing is scheduled right now." };
    const summary = items
      .slice(0, 8)
      .map(
        (item) =>
          `• ${item.title || item.id} — ${item.publish_at || "no time"} — ${item.status || "—"}`,
      )
      .join("\n");
    ...
    return {
      ok: false,
      reply: `Could not fetch schedule: ${error instanceof Error ? error.message : "unknown error"}.`,
      error: String(error),
    };
  }
}
...
async function dashboardLink(): Promise<YouTubeControlResult> {
  ...
  if (!info.reachable) {
    return {
      ok: false,
      reply: "The YouTube agent is not running, so I can't show its dashboard.",
      error: info.error,
    };
  }
  return {
 ...
 */
export async function runYouTubeControl(
  command: YouTubeControlCommand,
): Promise<YouTubeControlResult> {
...
function statusResult(info: YouTubeAgentInfo): YouTubeControlResult {
  ...
  const setup = health.setupRequired
    ? " (setup mode — needs YouTube credentials)"
    : "";
