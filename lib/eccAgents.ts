...
const CACHE_PATH =
  "/Users/bhavyarajput/Downloads/Rox/.rox-data/ecc_agents.json";
...
function parseAgentFile(filename: string, content: string): ECCAgent | null {
  ...
  const whenToUse =
    extractFrontmatter(fm, "when_to_use") ||
    extractFrontmatter(fm, "when") ||
    description;
...
export async function routeToECCAgent(
  task: string,
  ...
  if (!best || best.score < 0.1) {
    return {
      agent: null,
      reason: "No suitable ECC agent found",
      confidence: 0,
    };
  }
