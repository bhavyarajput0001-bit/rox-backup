...
async function migrateTranscript(items: MemoryItem[]): Promise<void> {
  ...
  const importantMessages = transcript
    .filter(
      (msg) =>
        msg.role === "rox" &&
...
export async function queryMemory(params: {
  ...
  const scored = items.map((item) => {
    ...
    // Content similarity
    const itemText =
      `${item.content} ${item.context ?? ""} ${item.tags.join(" ")}`.toLowerCase();
    for (const token of queryTokens) {
  ...
  for (const item of items) {
    ...
    if (time > lastHour) {
      if (!recentByCategory[item.category]) {
        recentByCategory[item.category] = {
          count: 0,
      ...
      if (
        time > new Date(recentByCategory[item.category].lastUpdated).getTime()
      ) {
...
function extractTags(text: string): string[] {
  return text
    ...
    .filter(
      (w) => !["about", "their", "there", "through", "another"].includes(w),
    );
}
