...
export async function readWebpage(url: string) {
  ...
  if (!response.ok)
    throw new Error(`Webpage reader returned ${response.status}.`);
...
export async function getNews(topic = "technology") {
  ...
  if (!response.ok)
    throw new Error(`News service returned ${response.status}.`);
  const xml = await response.text();
  return [
    ...xml.matchAll(
      /<item>[\s\S]*?<title>([\s\S]*?)<\/title>[\s\S]*?<link>([\s\S]*?)<\/link>[\s\S]*?<\/item>/g,
    ),
  ]
    .slice(0, 5)
    .map(
      (match, index) =>
        `${index + 1}. ${match[1].replace(/<!\[CDATA\[|\]\]>/g, "").trim()} - ${match[2].trim()}`,
    )
    .join("\n");
}
...
export async function searchWebCompact(
  query: string,
    ...
    if (Array.isArray(results)) {
      return results
        .map((item) => ({
          ...
          summary: String(
            item?.summary ?? item?.description ?? item?.snippet ?? "",
          ).slice(0, 400),
        }))
