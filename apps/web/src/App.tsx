...
export default function App() {
  ...
  if (!booted) {
    return (
      <BootScreen show={showBoot} onSkip={handleSkipBoot} />
    );
  }
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden bg-rox-bg">
      ...
      <div className="absolute inset-0 pointer-events-none" style={{
        background: 'radial-gradient(ellipse at 50% 50%, rgba(255,138,0,0.06) 0%, transparent 60%)',
      }} />
      <div className="absolute inset-0 pointer-events-none" style={{
        background: 'radial-gradient(circle at 30% 40%, rgba(255,100,0,0.04) 0%, transparent 40%)',
      }} />
      ...
      {/* Main Layout */}
      <div className={`relative z-10 w-full h-full flex flex-col ${focusMode ? 'focus-dim' : ''}`}>
        <TopBar />
          ...
          <div className="flex-1 flex flex-col items-center justify-center pointer-events-none">
            ...
            {!focusMode && (
              <div className="pointer-events-auto flex gap-4">
                <FloatingAction icon="lightbulb" label="Ideas" sublabel="Turn thoughts into plans" />
                <FloatingAction icon="sparkles" label="Create" sublabel="Write, design, build" />
                <FloatingAction icon="search" label="Research" sublabel="Find what matters" />
                <FloatingAction icon="zap" label="Execute" sublabel="Make it real" />
              </div>
            )}
          </div>
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="relative"
      >
        <div className="w-4 h-4 rounded-full bg-rox-amber" style={{
          boxShadow: '0 0 60px 30px rgba(255,138,0,0.6), 0 0 120px 60px rgba(255,138,0,0.2)',
        }} />
      </motion.div>
...
function FloatingAction({ icon, label, sublabel }: { icon: string; label: string; sublabel: string }) {
